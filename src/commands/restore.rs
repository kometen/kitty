...
pub fn restore_files(options: Option<RestoreOptions>) -> Result<(), KittyError> {
    ...
    let files_to_process: Vec<&TrackedFile> = match &options.path {
        Some(path) => {
            ...
            let matching_files: Vec<&TrackedFile> = repository
                .files
                .iter()
                .filter(|f| Path::new(&f.original_path) == file_path || f.original_path.contains(path))
                .collect();
    ...
    for file in &files_to_process {
        ...
        let encrypted_stored_content = match fs::read(repo_path.join(&file.repo_path)) {
            Ok(content) => content,
            Err(e) => {
                println!("  {} Could not read repository file: {}", "ERROR:".red().bold(), e);
                error_count += 1;
                continue;
            }
        };
        ...
        if let Some(parent) = file_path.parent() {
            if !parent.exists() {
                ...
                if let Err(e) = fs::create_dir_all(parent) {
                    println!("  {} Failed to create directory: {}", "ERROR:".red().bold(), e);
                    error_count += 1;
                    continue;
                }
            }
        ...
        if file_exists && options.backup {
            ...
            match fs::copy(file_path, &backup_path) {
                Ok(_) => {},
                Err(e) => println!("  {} Failed to create backup: {}", "WARNING:".yellow().bold(), e)
            }
        }
        ...
        if needs_privileges {
            ...
            println!("  {} This file may require elevated privileges to modify.", "NOTE:".yellow());
